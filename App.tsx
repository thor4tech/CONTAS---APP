...
import { Activity, ShieldAlert, CheckCircle2, CreditCard, Star, ShieldCheck, Zap } from 'lucide-react';
...
const App: React.FC = () => {
      ...
      if (u) {
        ...
        if (!docSnap.exists()) {
          ...
          const newProfile: UserProfile = {
            ...
            planId: 'PRO', // Trial inicia no PRO para experiência completa
  ...
  // Função Central de Verificação de Status
  const getAccessStatus = () => {
    if (!profile) return { isLocked: false, message: '' };
    
    // Se estiver ATIVO ou PENDENTE (atrasado), permite acesso
    if (profile.subscriptionStatus === 'ACTIVE' || profile.subscriptionStatus === 'PENDING') {
      return { isLocked: false, status: profile.subscriptionStatus };
    }

    // Se estiver em TRIAL, verifica data
    if (profile.subscriptionStatus === 'TRIAL') {
      const now = new Date();
      const end = new Date(profile.trialEnd);
      const isExpired = now > end;
      return { isLocked: isExpired, status: 'TRIAL', expired: isExpired };
    }

    // Caso CANCELADO ou REEMBOLSADO
    return { isLocked: true, status: profile.subscriptionStatus };
  };
  ...
  // Middleware Visual: Bloqueio de Plano Inativo ou Trial Expirado
  if (user && profile && access.isLocked) {
...
// Interface de Upgrade (Bloqueio) com links oficiais da Kiwify
const BillingLock = ({ profile, onSignOut }: { profile: UserProfile, onSignOut: () => void }) => (
  <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 font-inter">
    <div className="max-w-5xl w-full bg-white rounded-[48px] shadow-4xl overflow-hidden flex flex-col md:flex-row border border-white/10">
        ...
        <div>
          ...
          <p className="text-slate-400 font-medium leading-relaxed mb-10">
            {profile.subscriptionStatus === 'TRIAL' 
              ? 'Seu período de 7 dias de teste gratuito chegou ao fim. Seus dados estão salvos e aguardando ativação.' 
      ...
      <div className="md:w-7/12 p-10 md:p-16 bg-white grid grid-cols-1 gap-6">
        <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Selecione seu plano de comando</h3>
        
        {/* ESSENCIAL */}
        <a href="https://pay.kiwify.com.br/CRTkeeH" target="_blank" className="p-8 rounded-[32px] border-2 border-slate-100 hover:border-indigo-600 transition-all group flex items-center justify-between">
          <div>
        ...
        {/* PRO */}
        <a href="https://pay.kiwify.com.br/56PjP10" target="_blank" className="p-8 rounded-[32px] border-2 border-indigo-600 bg-indigo-50/50 relative shadow-xl shadow-indigo-100 flex items-center justify-between group">
          <div className="absolute top-0 right-10 -translate-y-1/2 bg-indigo-600 text-white px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest flex items-center gap-2">
        ...
        {/* MASTER */}
        <a href="https://pay.kiwify.com.br/Lkgu7yS" target="_blank" className="p-8 rounded-[32px] border-2 border-slate-100 hover:border-slate-900 transition-all group flex items-center justify-between">
          <div>
